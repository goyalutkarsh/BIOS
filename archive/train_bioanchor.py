...
def draw_fig(loss, cancer_type, epoch):
    ...
    plt.savefig(f'results/{cancer_type}_Train_loss.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_bio", type=float, default=0.1, help="Bio-anchor loss weight")

    # ── No hardcoding: specify bio_dim and bio_anchor_file at runtime ──────────
    ...
    parser.add_argument("--bio_anchor_file", type=str, default=None,
                        help="Path to bio-anchors CSV (e.g. bio_anchors_BRCA_6dim.csv). "
                             "If not provided, trains without bio-anchor supervision.")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for step, batch_data in enumerate(DL):
            ...
            # Bio-anchor loss (use ALL dims — no random sampling)
