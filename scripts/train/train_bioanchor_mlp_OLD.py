...
class BioAnchorHeadMLP(nn.Module):
    """
    Scenario 2: MLP bio-anchor head.
    bio_dim -> hidden_dim (ReLU) -> bio_dim
    Can learn interactions between bio dimensions.
    """
    def __init__(self, bio_dim, hidden_dim, n_anchors):
        super().__init__()
...
def draw_fig(loss, cancer_type, epoch):
    ...
    plt.title('Scenario 2 (MLP) - Train loss vs. epoch')
    os.makedirs('results', exist_ok=True)
    plt.savefig(f'results/{cancer_type}_mlp_loss.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bio_hidden_dim", type=int, default=64,
                        help="Hidden layer size for MLP bio-anchor head")
    ...
    print(f"MLP Bio-Anchor Head (bio_dim -> {args.bio_hidden_dim} -> bio_dim)")
    ...
    # Scenario 2: MLP head with hidden layer
    bio_head = BioAnchorHeadMLP(
        bio_dim=args.bio_dim,
        hidden_dim=args.bio_hidden_dim,
